...
async function main() {
  ...
  const allModels = Object.keys(prisma).filter(k => !k.startsWith('_') && !k.startsWith('$') && k !== 'constructor');
  console.log('All Prisma models:', allModels);
  ...
  await prisma.supplierApiKey.deleteMany();
  await prisma.supplierAddon.deleteMany();
  ...
  console.log(`✅ Created ${tourists.length + supplierUsers.length + 1} users`);
  ...
  const tours = await Promise.all([
    ...
    prisma.service.create({
      data: {
        name: 'Wine Tasting Tour Pelješac',
        slug: 'wine-tasting-tour-peljesac',
        description:
          'Visit three family wineries on Pelješac peninsula with traditional lunch.',
        type: 'TOUR',
        supplierId: suppliers[1].id,
        locationId: locations[0].id,
        price: 85,
  ...
  console.log('✅ Created 2 premium subscriptions');

  // 6. Create Supplier Premium Add-ons
  console.log('⭐ Creating supplier add-ons...');
  await Promise.all([
  ...
  console.log('✅ Created 3 supplier add-ons');
  ...
  console.log('✅ Created 3 user preferences');
  ...
  console.log('✅ Created 5 user interactions');
  ...
  console.log('✅ Created social features');
  ...
  await Promise.all([
    ...
    prisma.notification.create({
      data: {
        ...
        message: 'Blue Cave & Island Hopping Tour price dropped to €85!',
  ...
  console.log('\n🚀 API running at: http://localhost:3333/api/v1');
